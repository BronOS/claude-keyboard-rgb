...
do {
    ...
    let (fp, _) = StripConfig.parse(["host": "h", "leds": 10, "fps": 4]); check(fp?.fps == 4, "fps parses")
    let (fpc, _) = StripConfig.parse(["host": "h", "leds": 10, "fps": 30]); check(fpc?.fps == 10, "fps clamps to 10")
    check(StripConfig.parse(["host": "h", "leds": 10]).0?.fps == 5, "fps defaults to 5")
    let (bwBad, bwErr) = StripConfig.parse(["host": "h", "leds": 10, "badgeWidth": 0]); check(bwBad == nil && bwErr != nil, "badgeWidth 0 rejected")
    let overlap = StripConfig(host: "h", leds: 10, statusRange: 0...9, badgeRange: 7...9, brightness: 1)
    let ov0 = stripColors(pic(.done, "static", badge: 0), overlap, colors, floor: 0.25)
    check(ov0.allSatisfy { eq($0, (0, 255, 40)) }, "overlapping ranges: without badges the status color fills the strip")
    let ov2 = stripColors(pic(.done, "static", badge: 2), overlap, colors, floor: 0.25)
    check(eq(ov2[6], (0, 255, 40)) && eq(ov2[7], (255, 120, 0)) && eq(ov2[8], (255, 120, 0)) && eq(ov2[9], (0, 255, 40)), "overlapping ranges: badges paint over the status color")
